...
STATE next_state(char tok) {
	...
	switch (tok) {
		...
		case '#':
			return COMMENT_S;
		default:
			if ('0' <= tok && tok <= '9') return NUMBER_S;
			if ('a' <= tok && tok <= 'z') return CHAR_S;
			if ('A' <= tok && tok <= 'Z') return CHAR_S;
			if ('_' == tok) return CHAR_S;
			return ERROR_S;
	}
}
...
#define UNGETCHAR() (tokstr.pop_back(), ch = tokstr.back() + ch)
	...
	while (state) switch (state) {
			...
			case STRING_S:
				...
				while ((NEXT_CHAR, STRING_S != next_state(CURCHAR) &&
				                       HALT_S != next_state(CURCHAR)))
					if ('\\' == CURCHAR) switch (NEXT_CHAR) {
							case 'n':
								tokstr.pop_back();
								tokstr.pop_back();
								tokstr += '\n';
			...
			case NUMBER_S:
				...
				while (NUMBER_S == next_state(CURCHAR)) NEXT_CHAR;
				...
				if (HALT_S != next_state(CURCHAR)) UNGETCHAR();
				return Token(INT, r, c, tokstr);
			case CHAR_S:
				DPRINT("Going to state CHAR_S");
				while ((NEXT_CHAR, CHAR_S == next_state(CURCHAR) ||
				                       NUMBER_S == next_state(CURCHAR)))
					;
				if (HALT_S != next_state(CURCHAR)) UNGETCHAR();
